...
* the internal method `$.jgrid.parseDate` are changed in the same way. The methods expect DOM of the grid as `this`. So if you used the methods *directly* in your code you should replace `$.jgrid.parseDate(...)` to something like `$.jgrid.parseDate.call($("#grid")[0], ...)`.
